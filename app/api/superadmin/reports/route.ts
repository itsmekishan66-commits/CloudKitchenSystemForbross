...
const fetchReports = async (days: number, customStart: string | null, customEnd: string | null) => {
  ...
  const expenseDateFilter = useCustomRange
    ...
    : sql`${transactions.type} = 'expense' and ${transactions.createdAt} >= date_sub(now(), interval ${days} day)`;
...
export async function GET(request: Request) {
  try {
    ...
    const cacheKey = useCustomRange
      ? [CACHE_TAGS.REPORTS, customStart, customEnd]
      : [CACHE_TAGS.REPORTS, String(intervalDays)];

    const getCachedReports = unstable_cache(
      () => fetchReports(intervalDays, customStart, customEnd),
      cacheKey,
      { revalidate: 120, tags: [CACHE_TAGS.REPORTS] }
    );
